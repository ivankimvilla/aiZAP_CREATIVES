                        ...
                        @foreach ($featuredItems as $project)
                            ...
                            <article class="project-card{{ $projectVideo ? ' has-video' : '' }}">
                                <div class="project-thumb" @if(!$projectVideo) style="background-image:url('{{ $projectImage }}')" @endif>
                                    @if($projectVideo)
                                        <video class="project-video" autoplay muted playsinline loop preload="metadata" poster="{{ $projectImage }}" src="{{ $projectVideo }}" data-src="{{ preg_match('/^\/\//', $projectVideo) ? 'https:'.$projectVideo : $projectVideo }}">
